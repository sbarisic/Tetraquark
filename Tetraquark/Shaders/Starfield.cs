...
namespace Tq {
	static partial class Shaders {
		...
		public static RenderStates UseStarfield(Vector2f Position, Vector2f Resolution,
			float Additive = 0.13f, float Subtractive = 0.04f, float Reductive = 0.2f, float Iterations = 4) {
...
vec3 GetStarColor(vec2 Pos, float Tresh) {
	...
	for (; i < iterations; i += 1)
		if (Rand(floor(gl_TexCoord[0].xy * res + (pos + Off * i) * (1.0 - reductive * i)) / res) > Tresh)
			Clr += Add - Sub * i;
